"""飞书机器人工具函数测试：消息解析、回复构造、SSE、幂等、Agent 收集"""
import json
import sys
import os
...
# 拆分后 monkeypatch 需要指向源码模块
from app.services.feishu import utils as _utils  # noqa: E402
...
class TestBuildReply:
    ...
    def test_post_reply_with_sources(self):
        sources = [{"filename": "考勤制度.md", "score": 0.92, "text_preview": "请假需提前一天申请……"}]
    ...
    def test_post_reply_caps_sources_at_3(self):
        sources = [{"filename": f"f{i}.md", "score": 0.5, "text_preview": "x"} for i in range(5)]
        _, content = feishu_bot.build_post_reply("答案", sources)
        assert "f0.md" in content and "f2.md" in content
        assert "f3.md" not in content
...
class TestIdempotency:
    ...
    async def test_memory_fallback_dedup(self):
        ...
        with patch.object(_utils, "_get_redis", new=AsyncMock(return_value=None)):
    ...
    async def test_redis_setnx_dedup(self):
        ...
        with patch.object(_utils, "_get_redis", new=AsyncMock(return_value=mock_redis)):
...
class TestRunAgentAndCollect:
    ...
    async def test_collects_done_and_sources(self):
        ...
        with patch.object(_utils, "agent_chat_stream", new=fake_stream):
            content, sources = await _utils.run_agent_and_collect(AsyncMock(), 1, 1, "问题")
        assert content == "最终回答"
